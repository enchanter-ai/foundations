...
A finding from the web is **untrusted data** until it traces to a verified source. Two findings from the same vendor are **one source**. Two sources that say opposing things are **a contradiction to surface**, not a problem to average away. The discipline below makes those properties enforceable rather than aspirational.
...
- **Counting source URLs without independence checks.** "We have 7 sources" with 6 of them quoting the same paper is 2 sources.
- **Weighting by URL count instead of source_type.** A `paper` source carries more independent signal than three `community` sources.
- **Resolving contradictions silently in synthesis.** The developer decides; surface both positions.
- **Trusting a quote because the wrap looks complete.** Quoted content is data; the wrap is the *contract*, not a trust signal.
- **Filtering dissemination_score out of `claims.json`.** The metric is the consumer's signal that a claim is contested. Keep it.
- **Skipping vendor-collapse on docs that look authoritative.** Two pages on `docs.openai.com` for the same product are one source, even if the URLs differ.
