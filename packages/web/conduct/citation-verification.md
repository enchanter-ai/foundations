...
Audience: any agent producing a cited artifact (`claims.json`, `report.md`, dossier, briefing) and the **verifier** subagent that confirms each cite is real. How to distinguish "I synthesized a coherent answer" from "I synthesized an answer the live web still supports."

## The rule

Self-coherence is not verification. A claim with `supporting: ["S3", "S7"]` proves the synthesizer cross-referenced two source IDs — it does not prove the cited *sources* actually back the claim, and it does not prove the cited URLs still exist on the live web.
...
Both tests are mechanical. If the verifier catches itself asking "is this fact interesting" or "is this fact true" — it has lost the contract. Trace check is shape-checking, not judgment.
...
**When it fires.** After Step 5b re-fetch passes, *before* the verdict is computed. Full depth only; skipped at quick depth. Defaults: `N = 10` top-confidence claims, `K = 3` re-framings per claim (≥ 1 negation, ≥ 1 paraphrase). N is configurable; raising N is cheap (Haiku tier, mechanical), so the floor matters more than the ceiling.
...
**Paraphrase quality contract.** Per `research-pipeline.md`'s query-diversity rule, paraphrases must be *substantively* different — different verbs, different scope, or explicit negation — not synonym swaps. A paraphrase set that's three near-identical re-wordings is a no-op pass and must be regenerated. The agent generating the re-framings (Haiku tier) follows mechanical patterns: (a) swap the action verb; (b) negate the predicate; (c) widen or narrow the scope. Three patterns, three outputs.
...
The retrieved-chunk range comes from the source record itself — fetchers that surface a code chunk MUST record its `chunk_start_line` and `chunk_end_line` on the source. If those fields are absent, the interval test cannot run; the verifier records `inconclusive` for that cite and falls back to the subject/action trace check.
...
**Honest-numbers framing — advisory, not a verdict gate.** The pre-check is a **strict-mechanical superset**: it has no paraphrase tolerance beyond the built-in synonym table, while Phase 6 Haiku will accept some flagged sentences as legitimate paraphrase or scope-narrowing. Treat the pre-check's `violations` list as a **rewrite worklist for the orchestrator**, not a verdict. Phase 6 is the verdict gate. A dossier that passes the pre-check still must clear Phase 6; a dossier that fails the pre-check may still clear Phase 6 on a subset of its flagged items — but the cheap fix is to rewrite first, dispatch after.

If the orchestrator skips the rewrite step and dispatches Phase 6 anyway (e.g., the synthesizer disputes the pre-check's reading), record the divergence in `trace.json#phase5_5` so post-hoc analysis can compare mechanical-superset coverage against Haiku semantic coverage. Do not suppress pre-check output to make the brief "look cleaner" — that's an honest-numbers violation.
...
- **Skipping the re-fetch at full depth because the fetcher returned recently.** The re-fetch protects against content drift between fetch and synthesis time — and it's the fabrication firewall.
- **Stretching subject/action matching to "feel right" rather than "match."** Lexical overlap isn't enough; tests A+B are mechanical.
- **Reporting `refetch_pass_rate` based on sampled URLs minus unreachables, presenting it as "this is fine."** The unreachables are a separate signal; flag them, don't disappear them.
- **Treating `pass-via-archive` and `pass` as equivalent in the consumer.** Archive passes mean the live web has rotted; surface that to the developer or downstream skill.
- **Re-fetching at orchestrator tier.** Re-fetch is a low-tier verifier subagent's job. See [web-fetch.md](./web-fetch.md).
